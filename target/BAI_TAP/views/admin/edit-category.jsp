...
<section class="category-form-panel">
    ...
    <form action="<c:url value='/admin/category/edit'/>" method="post" enctype="multipart/form-data">
        ...
        <c:if test="${not empty category.icon}"><c:url value="/image" var="imageUrl"><c:param name="fname" value="${category.icon}"/></c:url><img class="category-image current-image" src="${imageUrl}" alt="${category.name}"></c:if>
        <div class="form-group"><label for="icon">&#7842;nh &#273;&#7841;i di&#7879;n m&#7899;i</label><input id="icon" type="file" name="icon" accept="image/png,image/jpeg,image/gif,image/webp"></div>
